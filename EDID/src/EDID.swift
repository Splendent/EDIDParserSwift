// EDID.swift
//
// from https://github.com/reitermarkus/DDC.swift/blob/master/DDC/EDID.swift
// extension, parsingVic parts from SP
//
...
internal extension UInt32 {
  init(highestByte: UInt8, highByte: UInt8, lowByte: UInt8, lowestByte: UInt8) {
    self.init(
        UInt32(UInt16(highByte: highestByte, lowByte: highByte)) << 16 |
        UInt32(UInt16(highByte: lowByte, lowByte: lowestByte))
    )
  }
}
...
internal extension UInt64 {
  init(_ byte1: UInt8, _ byte2: UInt8, _ byte3: UInt8, _ byte4: UInt8, _ byte5: UInt8, _ byte6: UInt8, _ byte7: UInt8, _ byte8: UInt8) {
    self.init(
        UInt64(UInt32(highestByte: byte1, highByte: byte2, lowByte: byte3, lowestByte: byte4)) << 32 |
        UInt64(UInt32(highestByte: byte5, highByte: byte6, lowByte: byte7, lowestByte: byte8))
    )
  }
}
...
public class EDID {
  ...
  public lazy var manufacturerId: UInt16 = { [unowned self] in UInt16(highByte: self.rawValue[8], lowByte: self.rawValue[9]) }()

  public lazy var productCode: UInt16 = { [unowned self] in UInt16(highByte: self.rawValue[11], lowByte: self.rawValue[10]) }()

  public lazy var serialNumber: UInt32 = { [unowned self] in UInt32(highestByte: self.rawValue[15], highByte: self.rawValue[14], lowByte: self.rawValue[13], lowestByte: self.rawValue[12]) }()
  ...
  public lazy var descriptors: [Descriptor] = { [unowned self] in
    [
  ...
  private static func detailedTimingInformation<T: Collection>(from data: T) -> Descriptor where T.Index == Int, T.Element == UInt8 {
    let pixelClock = UInt16(highByte: data[1], lowByte: data[0])
    ...
    timingInformation.horizontalActive = UInt32(UInt16(highByte: data[4] >> 4, lowByte: data[2]))
    timingInformation.horizontalBlanking = UInt32(UInt16(highByte: data[4] & 0b1111, lowByte: data[3]))

    timingInformation.verticalActive = UInt32(UInt16(highByte: data[7] >> 4, lowByte: data[5]))
    timingInformation.verticalBlanking = UInt32(UInt16(highByte: data[7] & 0b1111, lowByte: data[6]))
      
    timingInformation.horizontalSyncOffset = UInt32(UInt16(highByte: data[11] >> 6, lowByte: data[8]))
    timingInformation.horizontalSyncPulseWidth = UInt32(UInt16(highByte: data[11] >> 4 & 0b11, lowByte: data[9]))
    ...
    timingInformation.horizontalScaled = UInt32(UInt16(highByte: data[14] >> 4, lowByte: data[12]))
    timingInformation.verticalScaled = UInt32(UInt16(highByte: data[14] & 0b1111, lowByte: data[13]))
